...
|------|----------|---------|
| Marketing / investor **site** (built) | `public/index.html`, `public/how-it-works.html` | Home, careers, long-form "How it works" |
| **Investor deck** (built) | `public/deck.html` + `public/investor-*/` slides | Full-screen deck and standalone slide HTML |
| **Shared UI** (source) | `src/assets/` | `deck.css`, `deck.js` (copied into `public/assets/` on build) |
| **Charts / data** (source) | `data/` | Source exports used by slides (copied into `public/data/` on build) |
| **How it works (source)** | `src/public/home/how-it-works-content/*.md` | Markdown; build with `python scripts/build-how-it-works.py` from the repo root |
