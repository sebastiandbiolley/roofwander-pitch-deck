...
Each slide uses a **`deck-section` + hook** (e.g. `.deck-section.market-context`, `.traction-slide`, `.system-diagram-root`) for animation and width overrides. Slides may add BEM-like prefixes for unique UI (e.g. `.market-size-card`, `.traction-card`, `.pos-graph`). Keep **intros and section shells** on shared `.deck-intro` / `.deck-title` / `.deck-desc`; only **layout and data viz** stay heavily custom.
